...
# Calculate popularity ratio (indegree/outdegree) for alters in both terms

# Calculate in-degree (number of nominations received) for each participant
# by counting how many times they appear as an alter
indegree_counts <- data %>%
  group_by(Term, alter) %>%
  dplyr::summarise(indegree = n(), .groups = "drop")

# Calculate out-degree (number of nominations made) for each participant
# by counting how many alters each ego nominated
outdegree_counts <- data %>%
  group_by(Term, ego) %>%
  dplyr::summarise(outdegree = n(), .groups = "drop")

# Join the indegree and outdegree counts
# Rename columns to match the join
indegree_counts <- indegree_counts %>%
  dplyr::rename(participant = alter)

outdegree_counts <- outdegree_counts %>%
  dplyr::rename(participant = ego)

# Join the datasets
popularity_data <- full_join(indegree_counts, outdegree_counts, by = c("Term", "participant"))

# Correctly handle missing values:
# - Participants who nominated others but weren't nominated: indegree = 0
# - Participants who received nominations but didn't nominate anyone: outdegree = NA
popularity_data <- popularity_data %>%
  mutate(
    indegree = ifelse(is.na(indegree), 0, indegree),
    # Keep outdegree as NA if it's missing - don't replace with 0
  )

# Calculate popularity ratio (indegree/outdegree)
# Only for participants with valid outdegree values
popularity_data <- popularity_data %>%
  mutate(
    popularity_ratio = case_when(
      is.na(outdegree) ~ NA_real_,  # If they didn't nominate anyone, ratio is NA
      outdegree == 0 ~ NA_real_,    # Avoid division by zero
      TRUE ~ indegree / outdegree   # Normal case: calculate ratio
    )
  )

# Calculate statistics by term
popularity_stats <- popularity_data %>%
  filter(!is.na(popularity_ratio)) %>% # Remove NA values
  group_by(Term) %>%
  dplyr::summarise(
    mean_ratio = mean(popularity_ratio, na.rm = TRUE),
    sd_ratio = sd(popularity_ratio, na.rm = TRUE),
    median_ratio = median(popularity_ratio, na.rm = TRUE),
    min_ratio = min(popularity_ratio, na.rm = TRUE),
    max_ratio = max(popularity_ratio, na.rm = TRUE),
    n = n()
  )

# Print the results
cat("Popularity Ratio Statistics (Indegree/Outdegree):\n")
...
ggplot(data, aes(x = loneEgo, fill = Term)) +
    ...
    #geom_vline(aes(xintercept=mean(loneEgo)),  colour = "black", linetype = "dashed") +
    theme_linedraw() + labs(x = "Ego's loneliness", y = "Count", title = "A", subtitle = "Distribution of Ego's Loneliness") + theme(plot.title = element_text(face = "bold"))
...
# Distribution of Alter's Popularity (Indegree/Outdegree ratio)
# Calculate indegree (nominations received)
indegree <- data %>%
  group_by(Term, alter) %>%
  dplyr::summarise(indegree = n(), .groups = "drop")

# Calculate outdegree (nominations made)
outdegree <- data %>%
  group_by(Term, ego) %>%
  dplyr::summarise(outdegree = n(), .groups = "drop") %>%
  dplyr::rename(alter = ego)  # rename to match with indegree

# Join to calculate popularity ratio
popularity <- full_join(indegree, outdegree, by = c("Term", "alter"))

# Handle missing values correctly:
# - Replace NA indegree with 0 (participants who nominated others but weren't nominated)
# - Keep NA outdegree as is (participants who received nominations but didn't nominate)
popularity <- popularity %>%
  mutate(
    indegree = ifelse(is.na(indegree), 0, indegree),
    # Calculate popularity ratio where possible
    popularity_ratio = case_when(
      is.na(outdegree) ~ NA_real_,  # If they didn't nominate anyone, ratio is NA
      outdegree == 0 ~ NA_real_,    # Avoid division by zero
      TRUE ~ indegree / outdegree   # Normal case: calculate ratio
...
# Plot distribution of alter's popularity (indegree/outdegree ratio)
q <- popularity %>%
  filter(!is.na(popularity_ratio)) %>%
  ggplot(aes(x=popularity_ratio, fill=Term)) +
    geom_histogram(color="#e9ecef", alpha=0.6, position = 'identity', binwidth = 0.2) +
    scale_fill_manual(values=c("#69b3a2", "#404080")) +
    # Add separate mean lines for each Term
    geom_vline(data = popularity %>% 
                 filter(!is.na(popularity_ratio)) %>%
                 dplyr::group_by(Term) %>% 
                 dplyr::summarise(mean_ratio = mean(popularity_ratio, na.rm=TRUE)),
               aes(xintercept=mean_ratio, color=Term), 
               linetype = "dashed", size=1) +
    scale_color_manual(values=c("#216843", "#202060")) +
    theme_linedraw() +
    labs(x = "Indegree/Outdegree - Alter's popularity", 
         y = "Frequency", 
         title = "B", 
...
```{r H1}
# ratio of the number of be nominated by the number of nominating others as estimation of popularity
data <- data %>%
  left_join(
    popularity %>% select(Term, alter, popularity_ratio),
    by = c("Term", "alter")
  )

r = lmer(popularity_ratio ~ loneEgo + (1|ego), data = data); summary(r)
r.squaredGLMM(r)
confint(r)

p <- ggplot(data, aes(y = popularity_ratio, x = loneEgo, color = Term)) +
  ...
  xlab("Ego's loneliness") + ylab("Alter's popularity (indegree/outdegree)"); p
```
...
fit <- lmer(recpPer ~ loneEgo* popularity_ratio + (1|ego), data); summary(fit)
r.squaredGLMM(fit)
confint(fit)
...
# Assuming you have a loneliness variable for egos called loneEgo
popular_saturation <- data_with_saturation %>%
  # Use popularity_ratio to determine if alter is popular (above median)
  dplyr::group_by(Term) %>%
  dplyr::mutate(is_popular = popularity_ratio > median(popularity_ratio, na.rm = TRUE)) %>%
  ungroup() %>%
...
r = lmer(popularity_ratio ~ rank + (1|ego), data = data2); summary(r)
r.squaredGLMM(r)
confint(r)

r = lmer(popularity_ratio ~ rank*loneEgo + (1|ego), data = data2); summary(r)
r.squaredGLMM(r)
confint(r)
```